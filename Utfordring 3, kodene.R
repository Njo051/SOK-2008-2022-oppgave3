...
Sys.setlocale(locale="no_NO") #Norsk alfabet tilgjengelig
...
sbtabell <- clean_names(sbtabell)#Rydder i navna
...
oppgave1<-sbtabell %>% 
  ...
  labs(subtitle="Figur 1",
       x = "Tid",
       y = "Arbeidsledighet i %",
       title = "Arebeidledighet for ungdommer og voksne",
       col = "") +
  theme_bw()+
  theme()+
...
#Kjører korrelasjonstest for de ulike landene for å kunne argumentere i oppgaven, har fått litt hjelp av medstudenter til korrelasjonstestene. 
